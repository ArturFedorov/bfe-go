package min_stack

import "testing"

func TestMinStack(t *testing.T) {
	t.Run("leetcode example push/pop/top/getMin", func(t *testing.T) {
		s := Constructor()
		s.Push(-2)
		s.Push(0)
		s.Push(-3)
		if got := s.GetMin(); got != -3 {
			t.Errorf("GetMin() = %d, want -3", got)
		}
		s.Pop()
		if got := s.Top(); got != 0 {
			t.Errorf("Top() = %d, want 0", got)
		}
		if got := s.GetMin(); got != -2 {
			t.Errorf("GetMin() = %d, want -2", got)
		}
	})

	t.Run("multiple mins", func(t *testing.T) {
		s := Constructor()
		s.Push(1)
		s.Push(1)
		s.Push(1)
		if got := s.GetMin(); got != 1 {
			t.Errorf("GetMin() = %d, want 1", got)
		}
		s.Pop()
		if got := s.GetMin(); got != 1 {
			t.Errorf("GetMin() after pop = %d, want 1", got)
		}
	})

	t.Run("pop min reveals next min", func(t *testing.T) {
		s := Constructor()
		s.Push(5)
		s.Push(3)
		s.Push(1)
		if got := s.GetMin(); got != 1 {
			t.Errorf("GetMin() = %d, want 1", got)
		}
		s.Pop()
		if got := s.GetMin(); got != 3 {
			t.Errorf("GetMin() after pop = %d, want 3", got)
		}
		s.Pop()
		if got := s.GetMin(); got != 5 {
			t.Errorf("GetMin() after second pop = %d, want 5", got)
		}
	})
}
